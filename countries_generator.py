...
def generate():
            ...
            if i["rel"] != "guest" and x["run"]["players"][0]["id"] == i["id"]:
                try:
                    for c in country_data:
                        if i["location"]["country"]["code"] == c["code"]:
                            c["players"].append({"name": i["names"]["international"],
                                                 "time": str(datetime.timedelta(seconds=int(x["run"]["times"]["primary_t"])))})
                            new_country = False
                    if new_country:
                        country_data.append({"code": i["location"]["country"]["code"],
                                             "name": i["location"]["country"]["names"]["international"],
    ...
    with open("countries.tsv", "w+") as output:
        result = result.replace("\u200b", "")
        result = result.replace("\u015a", "")
        result = result.replace("\u0119", "")
        output.write(result)
        output.close()
